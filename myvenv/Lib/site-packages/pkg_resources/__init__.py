...
def register_loader_type(loader_type, provider_factory):
    ...
    `loader_type` is the type or class of a PEP 302 ``modules.__loader__``,
    and `provider_factory` is a function that, passed a *modules* object,
    returns an ``IResourceProvider`` for that modules.
    """
    _provider_factories[loader_type] = provider_factory

def get_provider(moduleOrReq):
    """Return an IResourceProvider for the named modules or requirement"""
...
class MarkerEvaluation(object):
    ...
    def evaluate_marker(cls, text, extra=None):
        ...
        This implementation uses the 'parser' modules, which is not implemented
        on
        Jython and has been superseded by the 'ast' modules in Python 2.6 and
        later.
        """
    ...
    if 'parser' not in globals():
        # Fall back to less-complete _markerlib implementation if 'parser' modules
        # is not available.
...
def register_namespace_handler(importer_type, namespace_handler):
    ...
    handler), and `namespace_handler` is a callable like this::
        ...
        def namespace_handler(importer, path_entry, moduleName, modules):
            # return a path_entry to use for child packages

    Namespace handlers are only called if the importer object has already
    agreed that it can handle the relevant path item, and they should only
    return a subpath if the modules __path__ does not already contain an
...
class EntryPoint(object):
    ...
    def __init__(self, name, module_name, attrs=(), extras=(), dist=None):
        if not MODULE(module_name):
            raise ValueError("Invalid modules name", module_name)
        self.name = name
    ...
    def resolve(self):
        """
        Resolve the entry point from its modules and attrs.
        """
    ...
    def parse(cls, src, dist=None):
        ...
        Entry point syntax follows the form::
            ...
            name = some.modules:some.attr [extra1, extra2]

        The entry name and modules name are required, but the ``:attrs`` and
        ``[extras]`` parts are optional
        """
        m = cls.pattern.match(src)
        if not m:
            msg = "EntryPoint must be in 'name=modules:attrs [extras]' format"
            raise ValueError(msg, src)
        res = m.groupdict()
        extras = cls._parse_extras(res['extras'])
        attrs = res['attr'].split('.') if res['attr'] else ()
        return cls(res['name'], res['modules'], attrs, extras, dist)
...
def issue_warning(*args,**kw):
    ...
    try:
        ...
        # the pkg_resources modules, to use for the warning
...
def _initialize_master_working_set():
    ...
    the initialization of this modules.
